...
export default function Login(props) {
    ...
    function validate() {
        if (loginInfo.username.length < 3) {
            ...
            Toast("Password Length Should Be Greater Than 8 Characters !")
            return false
        }
        return true
    }

    async function validateUserAndEmail() {
    ...
    async function handleNextClick() {
        if (validate() && await validateUserAndEmail()) {
            axios.get(`${BASE_URL}/api/image?username=${loginInfo.username}`)
                .then(res => {
    ...
    function login() {
        ...
        if (loginInfo.pattern.length < 4) {
            Toast("Chose atleast 4 images !")
            return
        }
        props.setLoading(true)
        axios.post(`${BASE_URL}/api/user/login`, loginInfo)
            .then(res => {
        ...
        <div className=" sm:h-[38rem] sm:mt-12">
            ...
            {!next && <div className="flex justify-center h-full">
                ...
                <div className="hidden sm:block">
                    <img className="transition duration-500 ease-in-out hover:scale-95 h-full" alt="" src="../static/img/signup.jpg"/>
                </div>
                {/*LOGIN FORM*/}
                <div className="font-['Work_Sans'] mt-16">
                    <p className="text-white text-3xl sm:text-5xl sm:font-bold px-4 sm:px-0">Login</p><br/>
                    <p className="text-white text-lg sm:text-2xl px-4 sm:px-0">Welcome Back! Enter Your login credentials Below</p>
                    ...
                    <button onClick={handleNextClick} className="ml-4 sm:ml-0 transition duration-500 ease-in-out h-8 sm:h-12 bg-[#000000] rounded-full px-6 sm:w-2/3 mt-6 text-white border-2 hover:bg-transparent border-[#000000] font-bold">Next</button>
                </div>
            </div>}

            {next && <div className="sm:flex justify-center h-full">
                <div className="hidden sm:grid grid-cols-4 bg-[#3B3B3B] h-full rounded-lg w-[75%] justify-items-center py-4 px-2 gap-2 ml-2">
                    {getIcons()}
                </div>
                ...
                <div className="sm:block hidden font-['Work_Sans'] mt-4 ml-16">
                    ...
                    <button onClick={login} className="transition duration-500 ease-in-out h-12 bg-[#000000] rounded-full px-6 w-2/3 mt-6 text-white border-2 hover:bg-transparent border-[#000000] font-bold">{getButtonTitle()}</button>
