...
export default function Navbar(props) {
    ...
    const additionalClasses = "text-[#000000]"
    ...
    return (
        <div className="md:p-6 py-2 px-4 flex justify-between">
            ...
            <div className="flex items-center cursor-pointer" onClick={() => window.location.reload()}>
                <img className="" width="24px" src="../static/img/banner.png" alt=""/>
                <p className="md:text-xl text-white ml-2 font-['Space_Mono']">GPA, <small>A New Way of Auth</small></p>
            </div>
            ...
            <div className="font-['Work_Sans'] text-white hidden md:flex items-center">
                ...
                {!props.loggedIn && <div>
                    <button onClick={() => setPage(Page.LOGIN_PAGE)} className="transition duration-500 ease-in-out bg-[#000000] rounded-lg px-4 py-1 ml-12 border-[#000000] border-2 hover:bg-transparent">Login</button>
                    <button onClick={() => setPage(Page.SIGNUP_PAGE)} className="transition duration-500 ease-in-out bg-[#000000] rounded-lg px-4 py-1 ml-6 border-[#000000] border-2 hover:bg-transparent">Sign Up</button>
                </div>}

                {props.loggedIn && <div className="flex">
                    <p className={`text-2xl font-mono text-[#A259FF]`}>{props.userInfo.username}</p>
                    <button onClick={() => logout()} className="transition duration-500 ease-in-out bg-[#000000] rounded-lg px-4 py-1 ml-4 border-[#000000] border-2 hover:bg-transparent">Logout</button>
                </div>}
            </div>
