...
export default function Signup(props) {
    ...
    function createAccount() {
        ...
        axios.post(`${BASE_URL}/api/user/signup`, signupInfo)
            .then(res => {
    ...
    function searchKeyword() {
        ...
        props.setLoading(true)
        axios.get(`${BASE_URL}/api/image/search?keyword=${keyword}`)
            .then(data => {
                props.setLoading(false)
    ...
    return (
        <div className=" sm:h-[38rem] mt-12">
            {!next && <div className="flex justify-center h-full">
                ...
                {/*SIGNUP FORM*/}
                <div className="font-['Work_Sans'] mt-4 m-5">
                    <p className="px-4 sm:px-0 text-white text-3xl sm:text-5xl sm:font-bold">Sign Up</p><br/>
                    <p className="text-white text-lg sm:text-2xl px-4 sm:px-0">Welcome! Enter Your Details and Signin by new Way</p>
                    ...
                    <button onClick={handleNextClick} className="ml-4 sm:ml-0 transition duration-500 ease-in-out h-8 sm:h-12 bg-[#000000] rounded px-6 sm:w-2/3 mt-6 text-white border-2 hover:bg-transparent border-[#000000] font-bold">Next</button>
                </div>
            </div>}

            {next && <div className="sm:flex h-full">
                {imageData.length > 0 && <div
                    className="hidden sm:grid grid-cols-4 bg-[#3B3B3B] h-full rounded-lg w-[75%] justify-items-center py-2 px-2 gap-2 ml-2">
                    {getIcons()}
                </div>}
                {imageData.length === 0 && <div
                ...
                <div className="hidden sm:block font-['Work_Sans'] mt-4 ml-12">
                    ...
                    <button onClick={createAccount} className="transition duration-500 ease-in-out h-12 bg-[#000000] rounded-full px-6 w-2/3 mt-12 text-white border-2 hover:bg-transparent border-[#000000] font-bold">{getButtonTitle()}</button>
                    <button onClick={handleBackClick} className="transition duration-500 ease-in-out border-2 border-[#000000] rounded-full px-4 h-12 ml-4 hover:bg-[#000000]">
                ...
                <div className="sm:hidden font-['Work_Sans'] mt-4 ml-4">
                    ...
                    <button onClick={createAccount} className="transition duration-500 ease-in-out h-8 bg-[#000000] rounded-full px-6 w-2/3 mt-12 text-white border-2 hover:bg-transparent border-[#000000]">{getButtonTitle()}</button>
                    <button onClick={handleBackClick} className="transition duration-500 ease-in-out border-2 border-[#000000] rounded-full px-4 h-8 ml-4 hover:bg-[#000000]">
                        <FontAwesomeIcon className="text-white" icon={faArrowLeft} />
                    </button>
                </div>
            </div>}
